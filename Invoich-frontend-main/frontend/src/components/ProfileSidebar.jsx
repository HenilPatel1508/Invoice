...
const ProfileSidebar = () => {
    ...
    const fetchProfileData = async () => {
      try {
        const response = await fetch(
          `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/user/getuserbyid/${userId}`,
          {
            method: "GET",
            headers: {
  ...
  const uploadImage = async (selectedImage) => {
    ...
    try {
      const response = await fetch(
        `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/business/updateBusiness/${businessId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
