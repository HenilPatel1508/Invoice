...
function Invoice() {
  ...
  const uploadCSV = async () => {
    ...
    try {
      const response = await fetch(
        "https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/invoice/bulkupload",
        {
          method: "POST",
          headers: {
  ...
  const deleteInvoice = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/invoice/deleteById/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
