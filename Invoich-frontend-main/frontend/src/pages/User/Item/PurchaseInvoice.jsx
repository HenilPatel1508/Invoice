...
export const PurchaseInvoice = () => {
  ...
  const uploadCSV = async () => {
    ...
    try {
      const response = await fetch(
        "https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/purchaseInvoice/purchaseinvoices/import",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this invoice?")) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/purchaseInvoice/deletepurchaseinvoice/${id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
