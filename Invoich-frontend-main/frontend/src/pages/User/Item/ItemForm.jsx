...
export const ItemForm = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      fetch(
        `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/item/getitembyitemid/${id}`
      )
        .then((response) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      try {
        const response = await fetch(
          id
            ? `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/item/updateitem/${id}`
            : `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/item/creatitem`,
          {
            method: id ? "PUT" : "POST",
            headers: {
