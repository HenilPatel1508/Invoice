...
export const Items = () => {
  ...
  const uploadCSV = async () => {
    ...
    try {
      const response = await fetch(
        "https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/item/bulkupload",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://invoich-backend-dot-nuxhealth-449518.el.r.appspot.com/api/item/deletitem/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
